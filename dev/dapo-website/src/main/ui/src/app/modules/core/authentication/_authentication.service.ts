...
export class AuthenticationService {
  ...
  public setToken(token:string) {
    ...
    this.broadcaster.$broadcast(AuthenticationEvent.AUTHENTICATION_SUCCESS);
    this.http.get("/user/me", {"headers": {"Authorization": `Bearer ${this.token}`}}).subscribe(
      result => {
        debugger;
        console.log(result);
      }
  ...
  public get decodedToken(): JwtToken {
    if (!this.token) return new JwtToken();
    return JSON.parse(AuthenticationService.b64DecodeUnicode(this.token.split('.')[1]));
  }
